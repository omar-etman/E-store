...
//to be able to get the dynamic slug route [slug]
import { useRouter } from 'next/router'
import useSWR from 'swr'
import { AppStateType, SingleProduct } from 'types'
// import {addCartItem} from ''
import { useDispatch, useSelector } from 'react-redux'
import { addCartItem } from 'redux/reducers/app'
...
export default function ProductPage() {
  ...
  //batala3 men el router el query bta3et el url
  const { slug } = router.query
  //frontend route eli hanro7lo
  //when you add a colon as below to a variable within a curly brace it changes its name , the type gets mentioned within the <>
  const { data: product, error } = useSWR<SingleProduct>(
    `/api/products/${slug}`
  )
  const cart = useSelector((state: any) => state.app.cart)
  const dispatch = useDispatch()
  const orderProduct = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
    if (product) {
      ...
      <main className="pt-10 sm:pt-8 md:pt-0">
        ...
        <div className="mx-auto max-w-2xl px-4 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:py-16">
          ...
          <div className="mt-4 lg:row-span-3 lg:mt-0">
            ...
            {/* <p className="text-3xl text-gray-900">{product.variants}</p> */}
            {/* Reviews */}
            <div className="mt-6">
